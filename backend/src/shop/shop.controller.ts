...
export class ShopController {
  ...
  @Get('config')
  async getStoreConfig(@Query('slug') slug?: string, @Query('domain') domain?: string) {
    if (!slug && !domain) {
  ...
  @Get('products/:slug')
  async getProduct(@Param('slug') slug: string, @Query('storeId') storeId: string) {
    if (!storeId) throw new NotFoundException('Store ID is required');
    return this.shopService.getProductBySlug(storeId, slug);
  }

  @Get('banners')
  async getBanners(@Query('storeId') storeId: string, @Query('position') position?: string) {
    if (!storeId) throw new NotFoundException('Store ID is required');
